	...
	return (
		<div ref={containerRef} className="relative flex flex-col items-center w-full">
			...
			<div className="relative w-full">
				<input type="text" name="artifact-search" placeholder="Enter artifact name..." autoComplete="off" value={input} onClick={handleSearchClick} onChange={handleChange} disabled={disabled} className="focus:outline-0 p-4 text-lg bg-neutral-800 rounded-lg w-full"/>{/* Search bar */}
				...
				{showSuggestions &&
					suggestions.length > 0 && ( // Show the list of suggestions when at least 1 exists and showSuggestions is true
						<ul className="absolute top-full left-0 right-0 max-h-64 overflow-y-auto bg-neutral-700 border border-gray-400 rounded-lg w-full z-10 cursor-pointer mt-2">
							{suggestions.map(
								(
									artifact // Create each suggestion as an li
								) => (
									<li key={artifact.name} onClick={() => handleSelect(artifact)} className="p-4 border-b border-gray-500 flex flex-row items-center justify-between hover:bg-neutral-600 transition">
										<img src={artifact.icon} className="h-16 w-16 sm:h-20 sm:w-20"></img>
										<span className="flex-1 text-center text-lg sm:text-xl">{artifact.name}</span>
									</li>
								)
							)}
						</ul>
